# AGENTS.md — Personal Dashboard App
...
Built and maintained primarily through Codex, in short sessions
...
**Disk is the source of truth: `.Codex/skills/` + `.Codex/skills/INDEX.md`.** Any
...
- **Scriptor** — `.Codex/skills/scriptor/` — all customer-facing sales copy; the doctrine stack lives in its references/ — pre-2026-07-03 (v2.2 as of 07-23)
- **Augur** — `.Codex/skills/augur-research/` — customer research, avatars, congregation maps — 2026-07-03 (v2.0.0 07-19)
- **Herald** — `.Codex/skills/herald-social/` — organic social, three locked pillars — 2026-07-03
- **Legatus** — `.Codex/skills/legatus-meta-ads/` — attended Meta Ads diagnosis + media buying; owns shared definitions (king goal, winner/potential, promo exclusion) — 2026-07-03 (v1.2.0 07-19)
- **Centurion** — `.Codex/skills/centurion-store-health/` — store health, checkout integrity — 2026-07-03
- **Faber** — `.Codex/skills/faber-creative-flywheel/` — the performance→creative flywheel: briefs off Legatus's winners/potentials, bloat guard, no fabricated proof — 2026-07-19 (v1.1.0)
- **Vigil** — `.Codex/skills/vigil-ads-monitor/` — unattended scheduled ad-account monitoring; reports, never diagnoses or mutates — 2026-07-19
- **Beowulf** — `.Codex/skills/beowulf-product-scouting/` — product scouting/validation before a
...
- Codex generates custom routines from goals + past performance
...
- These are written character voices (tone, phrasing, humor) layered over
  Codex's responses. Build them as text/prompt personas first — that's fully
...
- **Version control set up (2026-06-30)**: git initialized, `.gitignore` already
  excludes `.env` + secrets. A project allowlist lives in `.Codex/settings.json`
...
- **Phase 2 (needs Supabase — now unblocked):** replace the simple reader with the
  Codex API (natural-language → actions) behind the same `interpret()` seam, and
...
- **`.env` is in the Write/Edit-tool deny list** (`.Codex/settings.json`) —
  intentional guard rail from an earlier session. Codex can't write/edit
